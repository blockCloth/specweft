...
- Capability Center：把 MCP、Skill、本地 CLI 工具统一成能力清单，展示状态、风险、权限、安装和运行命令
- Project Registry：用 `~/.specweft/projects.json` 记录多个本地项目，一个 Web UI 可以切换管理
...
- MCP Server：让 Claude/Codex 通过 MCP 工具读取项目画像、能力中心、装配配置、review、recall，并应用 MCP/Skill
- Web UI：提供项目画像、能力中心、项目选择、runtime assembly、review、recall、MCP 接入配置的本地控制台
- 市场 Skill：从 skillsmp 搜索候选，做 Java/JavaScript 相关性过滤，提示本地规范冲突，并支持用户确认后安装到全局 Skill 池再启用到当前项目
...
flowchart TB
    ...
    subgraph Package["specweft npm package"]
        CLI["CLI\ninit/status/recommend/capabilities/review/recall"]
        Web["Local Web UI\nCapability Center / Memory / Review"]
